            ...
            <div class="card-body">
                <form method="POST" action="{{ route('product.store') }}" aria-label="{{ __('new-product') }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6">
                            <select class="form-control" name="partner_id" id="" required>
                                <option value=""></option>

                                @foreach($partners as $partner)
                                    <option value="{{ $partner->unid }}" {{ old('partner_id')===$partner->unid?'selected':'' }}>{{ $partner->name }}</option>
                                @endforeach
                        ...
                        <div class="col-md-6">
                            <select class="form-control" name="category_id" id="" required>
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->unid }}" {{ old('category_id')===intval($category->unid )?'selected':'' }}>{{ $category->name }}</option>
                                @endforeach
